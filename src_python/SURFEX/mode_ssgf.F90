...
REAL, DIMENSION(:), INTENT(IN) :: PRHOA
!
REAL, DIMENSION(SIZE(PUSR)), INTENT(OUT) :: PFM    ! Spray mass flux (kg m-2 s-1)
REAL, DIMENSION(SIZE(PUSR)), INTENT(OUT) :: PSV    ! normalized source volume m/s NB not used subsequently for now
REAL, DIMENSION(SIZE(PUSR)), INTENT(OUT) :: PSA    ! normalized source area 1/s
REAL, DIMENSION(SIZE(PUSR)), INTENT(OUT) :: PVFM   ! estimate mean fall velocity (m/s)
!
!* 0.2 declarations of local variables
!
